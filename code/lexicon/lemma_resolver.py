...
Two public entry points:
    ...
    resolve_one(form, prefer_pos=None)
...
This module never embeds and never reads sgf_lexicon directly. It only
reads lemma_form (built by build_lemma_forms.py) and optionally
...
def resolve(form, db_path):
    ...
    # If the form matches a known lemma, treat it as itself first. We
    # detect this by checking whether the form appears as a lemma in
    # any cached entry's candidate list. Cheap because the cache is in
    # RAM.
...
def expand_to_lemmas(form, db_path, prefer_pos=None):
    ...
    If a form has 3 candidates [(burn,v), (burn,n), (burned,adj)],
    this returns ['burn', 'burned'].
    """
    seen = set()
    out = []
