...
#!/usr/bin/env python3
"""
bm25_score.py — BM25 lexical scoring + cascade decision logic + divergence detection.

This module is a standalone stage in the search server's cascade. It does NOT
maintain a precomputed inverted index. Instead, it scores BM25 over whatever
small candidate set the prior stages handed it (typically 3 to 50 senses).
That keeps the lexicon storage simple and avoids index drift when entries
are updated or new sources imported.
...
Public functions:
    ...
    normalized_margin(scores) -> float
    cascade_should_fire(stage_cfg, prior_scores) -> bool
    candidates_diverge_on(candidates, axes) -> bool
    score_candidates(query_text, candidate_texts, bm25_cfg) -> list[float]

ARCHITECTURAL NOTE
==================
This module is based on the original GLEAN search server's bm25_score.py
but simplified for Synapedia v7. Key differences:

  1. Porter stemmer is inlined (no external dependency)
  2. Tokenization handles Synapedia's embedding_text format
  3. cascade_should_fire() accepts Synapedia's stage config shape
  4. No assumption about pos_simple vs pos_ud (uses whichever the caller provides)

No regex. No external models. Deterministic. Fast. Testable in isolation.
"""
...
def tokenize(text, lowercase=True, stemmer="porter"):
    """Deterministic tokenizer. Handles Synapedia's embedding_text format
    (pipes, colons, underscores are treated as punctuation and replaced
    with spaces)."""
    if text is None:
        return []
    s = str(text)
    ...
    # Also split on pipes and underscores (common in canonical IDs and embedding_text)
    for sep in ("|", "_", ":"):
        s = s.replace(sep, " ")
...
if __name__ == "__main__":
    # Quick smoke test
    query = "bank financial river"
    docs = [
        "bank financial institution that accepts deposits",
        "bank sloping land beside a river",
        "bank building where financial services are offered",
    ]
    cfg = {"lowercase": True, "stemmer": "porter"}
    scores = score_candidates(query, docs, cfg)
    print(f"BM25 scores: {scores}")
    print(f"Normalized margin: {normalized_margin(scores):.4f}")
    print(f"Should fire (when_tight, thresh=0.05): {cascade_should_fire({'mode': 'when_tight', 'margin_threshold': 0.05}, scores)}")
